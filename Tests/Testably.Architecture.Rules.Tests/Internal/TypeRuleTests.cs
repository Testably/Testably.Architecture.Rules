...
public sealed class TypeRuleTests
{
	...
	public void Constructors_ShouldApplyAllTypeFilters()
	{
		...
		result.Errors.Should().Contain(e => ((ConstructorTestError)e).Constructor.DeclaringType == type1);
		result.Errors.Should().NotContain(e => ((ConstructorTestError)e).Constructor.DeclaringType == type2);
	}
	...
	public void Constructors_ShouldFilterOutConstructorsFromTypes()
	{
		...
		result.Errors.Should().AllBeOfType<ConstructorTestError>();
		result.Errors.Should().Contain(e => ((ConstructorTestError)e).Constructor.DeclaringType == type1);
		result.Errors.Should().Contain(e => ((ConstructorTestError)e).Constructor.DeclaringType == type2);
	}
